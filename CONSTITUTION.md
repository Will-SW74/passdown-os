...
本 repo 中框架檔案、資料夾與工具部署實體的用途，以及「什麼時候該讀」：
...
| `RUBRICS.md` | 判斷 checklist（何時升級／算完成／問使用者／換路／驗品質／獨立判斷），各附正反例 | 對「做不做、停不停、算不算完成」，或是否接受他人主張猶豫時 |
...
| `memory/conventions.md` | 框架預設文風與專案慣例 | 要寫對外文字、長文件，或不確定某個風格/做法是否有既定慣例時查閱；日常短回覆只需遵守第 11 節的文風摘要 |
...
| `.codex/hooks.json` | 本 repo 自己的 Codex 專案層 hook 部署實體；下游專案不得直接複製，應由 `entrypoints/hooks/codex-hooks.json.example` 生成 | 維護或驗證本 repo 的 Codex hooks、執行 `/hooks` trust、或比對下游範本時 |
...
4. **主動防禦存檔線（可量測時 60%；不可量測時第 15 輪）**：只有 `DISPATCH.md` 第 7 節查證表列出可操作的「當前使用率量測來源」時，才以該來源顯示的 60% 為觸發線；沒有量測來源時，**對話達第 15 輪就是唯一有效的強制存檔線**。此時先執行結束協定存檔，接著直接提醒使用者：「context 已達存檔線，交接已寫好，建議重開一個全新 session 接手執行本專案」。優先由使用者重開新 session；只有使用者明確表示要在原對話繼續時，才啟動壓縮機制（例如 cc 的 `/compact`）。不可未提醒使用者就自行壓縮，也不可不存檔硬撐。
5. **強制硬紅線（有量測來源時 70%）**：只有當前使用率可由查證表所列外部來源讀出時，70% 才是硬紅線；一旦實測達到 70%，**必須立刻停止所有開發工作，將剩下預算用於執行「Session 結束協定」存檔**。沒有量測來源時不得猜百分比，依第 15 輪存檔線執行。
...
各輪數門檻的用途區分（多條同時適用時以先到者為準）：**10 輪／20 輪**是「規劃 task 大小」的目標值（見下方 Task 切分準則）；**可量測時 60%，不可量測時第 15 輪**是「執行中主動存檔」的觸發線；**12-15 輪**是「會自動截斷的環境」的硬性重開線。外部 tool-call checkpoint 只是較早提醒，**不可把工具呼叫次數換算成 context 百分比**。

**量測誠實原則**：名目 Context Window 容量只說明上限，不代表目前用了多少。模型不得靠內省估計 60%/70%，也不得以名目容量、回合長度或工具呼叫數反推百分比；有效來源與查證日期只維護在 `DISPATCH.md` 第 7 節查證表。
...
4. **Context 預算制（核心限制）** — **Task 的規模必須與接手 Agent 的 Context Window (CT) 深度掛鉤**。在對話長度達到該 Agent 的「實用上限（通常為 40% ~ 50% 滿）」前，必須能夠完整完成並驗證該 Task。
   ...
   - **動態拆分**：在執行過程中，一旦發現遇到預期之外的 bug 或複雜重構，導致對話輪數迅速上升、Context 消耗過快（有量測來源時達 CT 一半；無量測來源時接近第 15 輪），**必須立即停止工作，執行「Session 結束協定」將當前進度存檔並將剩餘部分拆成新 Task**，絕不可在快飽和的 Context 中硬撐。
...
1. **檢查並掛上會話鎖（強制首個動作）**：
   - **先檢查** `sessions/.active_lock` 是否存在。**存在** → 有兩種可能，**必須先分辨、不可直接覆寫**：
     - **另一個 agent 現在正在工作**（並行防護）：讀出鎖裡的 agent 代號與時間，**向使用者確認**：「偵測到 <agent> 於 <時間> 掛的會話鎖，請確認它是否還在工作？」使用者說還在 → **立刻停止，不要動任何檔案**，等對方跑完結束協定摘牌後再開始——兩個 agent 同時改同一個工作目錄是本框架防不了的，必須在門口擋下。
     - **上一個 session 異常中斷**（使用者確認沒有別人在跑，或鎖已明顯過時）：記下此事實，第 5 步復原協定會用到。
...
- **認知獨立**：使用者決定目標、優先序與風險取捨；但使用者、其他 agent、既有文件與 review finding 對事實、原因、嚴重度及解法的主張，都只是待驗證輸入，不是自動成立的結論。不得為迎合而接受，也不得為顯得獨立而刻意反對。需要接受、挑戰或修正他人主張時，依 [`RUBRICS.md`](RUBRICS.md)「接受他人主張前的獨立判斷」執行。
