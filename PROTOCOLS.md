...
**同機並行（最危險，優先預防）**：兩個 agent 同時在**同一個工作目錄**開工，是本框架防不了的情境——git 紀律只保護「不同 branch」，擋不住 live 共改。防線是開始協定第 1 步的會話鎖並行檢查：看到別人的鎖先問使用者，確認對方還在跑就停止等待。使用者側的紀律：**同一時間只讓一個 agent 動 repo，交接靠 CURRENT.md 換棒**——想「順便問另一個 agent」時，先讓現任跑完結束協定摘牌。
