...
### 來源端 passdown-lint 命令契約

新增來源 repo 的 tools/passdown-lint.py，只使用 Python 標準函式庫。介面為 python tools/passdown-lint.py [--root PATH] [--json]；預設 root 是目前 repo。安裝 agent 從來源位置以 --root 指向目標，檢查完成後不把 tools/ 複製進下游，也不在交接流程呼叫。成功輸出摘要並回傳 0，任何必要檢查失敗時逐項列出穩定檢查代碼與相對路徑並回傳 1。--json 輸出物件包含 ok、root、checks、errors，其中每個 error 包含 code、path、message。
...
- Markdown 不得連回自身；sessions/INDEX.md 的正式資料列必須各有一個存在且不重複的 sessions/ 內連結，且不得保留範例或省略列。
- handoff/CURRENT.md 的 Direct Memory Source 與 Context Index 所列 repo 相對目標存在。
- entrypoints/hooks/*.sh 不含 CRLF。

INSTALL.md 把此命令列為第 5 節必要驗收，但執行者是安裝 agent、程式位置是來源 repo；目標 payload 不含 tools/，日常交接不執行 lint。若 Python 門檻未通過，安裝本來就必須中止，因此不另設靜默降級。
...
- 從來源執行 python tools/passdown-lint.py --root <target>/passdown-os，對不含 tools/ 的乾淨目標回傳 0；刪除 .gitattributes、把 hook JSON 改壞、在 CURRENT.md 留佔位符、加入不存在的本地連結或把 shell 腳本改成 CRLF 時回傳 1。
...
- 對必要檔案、hook JSON、佔位符、斷鏈、自連結、session index、memory anchor 與 CRLF 故障建立暫存副本測試，確認命中預期檢查代碼且 exit code 為 1。
