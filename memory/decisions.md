...
**Why:** 主題級摘要只回答「改了哪些類別、為什麼」，原作者回來時仍無法一眼看出自己的 task 去哪，被迫逆向工程 git diff。實例：codex 把 s3-s4 tasks 由 cc 的 39 改為 42（58 增/51 刪的重寫），group 3 由 10→3、Task 5.6(BSOD)→4.4、1.1 拆成 1.1a＋8.1，但交接只有主題級敘述、無對應表 → 原作者卡在「看不出我的東西被改去哪」。附對應表即消除此摩擦。

**Agent:** cc（2026-07-22 13:40）— 使用者授權定案。

## PDOS-D-20260722-4 — 框架維護順序政策：熱修-backport 為預設、母庫-first 為選用、碰母庫必 commit+push

**Decision:** 維護框架碼（空間分工見 PDOS-D-20260722-3）有兩條 sanctioned 路徑，依觸發情境擇一，不得硬選單一種：
- **Order A（預設）— 專案端熱修後 backport**：框架在專案工作途中咬人時，就地在 root 熱修、繼續工作，試用無誤後 backport 回母庫並 push。不因框架故障停下整個 task。**代價是漂移風險 → 熱修但尚未回母庫前，必須在 `handoff/CURRENT.md` 的 Open items／blockers 記「backport 待辦」，收工未清就持續掛著提醒。**
- **Order B（選用）— 母庫先修再 pull**：非被卡、屬「刻意改進框架」時，直接在母庫改、pull 下專案，母庫恆為權威、零漂移。
- **硬規則：任何動到母庫的改動＝當次 commit + push。** 未 commit 的母庫改動是最危險狀態（專案看不到、其他 clone 帶不到、重 clone 即失）。母庫的權威記錄是它自己的 git 歷史＋`decisions.md`，**不是**專案逐字稿——逐字稿只是敘事冗餘，不可當母庫帳本。

**Why:** 硬選單一路徑都會製造摩擦——強推 B 會讓被卡時還得繞整套流程（正是「每 session 先 tune pd os」的來源），強推 A 會累積漂移。定死「情境→路徑」＋backport 硬規則＋待辦記錄，才能同時壓低「開洞、重工、修 pd os」三種風險。

**Agent:** cc（2026-07-22 13:30）— 使用者授權定案。
